...
function downloadAndUploadVideo(message, videoName, url, statusMessage, sendToDM, additionalContent, clipTimeframe) {
    ...
    exec(`${YT_DLP_PATH} --print "%(title)s - %(uploader)s" --no-playlist "${url}"`, async (error, stdout, stderr) => {
        ...
        const command = `${YT_DLP_PATH} -o ${videoName} --no-playlist ${clipParameters} -S "size:${MAX_VIDEO_SIZE}" --merge-output-format mp4 "${url}"`;
    ...
    execute: async (message, args) => {
        ...
        if (args.includes('-clip')) {
            ...
            if(args.length + 2 > index){
                ...
                let splitParanmeter = nextElement.split('-');
        ...
        try {
            const statusMessage = await message.reply('Downloading video...');
            console.log(`[!video] Info: Starting download...`);

            const videoName = `./temp/${message.author.id}_${Date.now()}.mp4`;
